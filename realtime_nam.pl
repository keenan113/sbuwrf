...
#Set Paths
	...
	$dirWPS		= "/D0/sbuwrf/REALTIME/WRFv3.9.1/WPS";
...
if (length("$init") == 2) {
	...
	$ENV{OUTDIR}		= "/D0/sbuwrf/REALTIME/NAM/$day";
	$outdir			= $ENV{OUTDIR};
...
#for ($FHR = 0; $FHR <= $runlength; $FHR+=$int) {
  ...
  for ($FHR = 0; $FHR <= $runlength; $FHR+=$int) {
	...
	if ($DOWNLOAD) {
		...
		$url = "http://nomads.ncep.noaa.gov/pub/data/nccf/com/nam/prod/nam.$YYYY$MM$DD";

		#Model grid file names (on server and renamed for our use).
		$file1 = "nam.t$HH"."z.awphys$FHR".".tm00.grib2";
		$file2 = "$YYYY$MM$DD$HH"."_0$FHR"."_nam.grb2";
	...
	if ($UNGRIB) {
		...
		for ($FHR = 0; $FHR <= $runlength; $FHR+=$interval) {
			if (length("$FHR") != 2) { $FHR="0$FHR"; }
			$gribfile = $outdir.'/'.$starttime.'_0'.$FHR.'_nam.grb2';
			print "$gribfile\n";
		...
		for ($dom = 1; $dom <= $max_dom; $dom++) {
			...
			while ($check == 0) {
				...
				if ( -e "$outdir/$wrfout" ) {
					...
					#Execute UPP.
					print ("cd $UPP_script_dir; $UPP_script_dir/run_unipost_ryan $outdir $sTime_unformatted $FHR d$domain");
		...
		for ($dom = 1; $dom <= $max_dom; $dom++) {
			...
			while ($check == 0) {
				...
				if ( -e "$outdir/$wrfout" ) {
					...
					@riphours = (0,1,3,24);
					for ($b=0;$b<=3;$b++) {
						my $riphour = int($hour) - int($riphours[$b]);
...
if ($NCL) {
	...
	for ($dom = 1; $dom <= $max_dom; $dom++) {
		...
		#Link NCL script.
		symlink "$infiles/ncl_timeseries_nam.ncl","$outdir/ncl_timeseries_nam.ncl";

		#Set environmental variable and run NCL script.
		system("cd $outdir; export wrf_dir=$outdir >/dev/null 2>&1; export wrf_dom=$domain >/dev/null 2>&1; ncl ncl_timeseries_nam.ncl >/dev/null 2>&1");

		#Hide the evidence.
		#unlink "$outdir/wrfout_d$domain.nc";
		unlink "$outdir/ncl_timeseries_nam.ncl";
		system("cd $outdir; mv *.png $outdir/images/ >/dev/null 2>&1");

		for ($s=0;$s<=$#nclstns;$s++) {
			system("cd $outdir; convert -trim $outdir/images/NAM_${nclstns[$s]}.000001.png $outdir/images/NAM_${nclstns[$s]}.000001.png");
			system("cd $outdir; convert -trim $outdir/images/NAM_${nclstns[$s]}.000002.png $outdir/images/NAM_${nclstns[$s]}.000002.png");
			system("cd $outdir; convert -trim $outdir/images/NAM_${nclstns[$s]}.000003.png $outdir/images/NAM_${nclstns[$s]}.000003.png");
			system("cd $outdir; convert -trim $outdir/images/NAM_${nclstns[$s]}.000004.png $outdir/images/NAM_${nclstns[$s]}.000004.png");
			rename "$outdir/images/NAM_${nclstns[$s]}.000001.png","$outdir/images/NAM_${nclstns[$s]}_tmp_d$domain.png";
			rename "$outdir/images/NAM_${nclstns[$s]}.000002.png","$outdir/images/NAM_${nclstns[$s]}_prs_d$domain.png";
			rename "$outdir/images/NAM_${nclstns[$s]}.000003.png","$outdir/images/NAM_${nclstns[$s]}_pcp_d$domain.png";
			rename "$outdir/images/NAM_${nclstns[$s]}.000004.png","$outdir/images/NAM_${nclstns[$s]}_wsp_d$domain.png";
		}
...
if ($FRONTPAGELOOP) {
	system ("cd $outdir/images/; convert -delay 50 -loop 0 NAM.refl_10cm.d01.0.gif NAM.refl_10cm.d01.3.gif NAM.refl_10cm.d01.6.gif NAM.refl_10cm.d01.9.gif NAM.refl_10cm.d01.12.gif NAM.refl_10cm.d01.15.gif NAM.refl_10cm.d01.18.gif NAM.refl_10cm.d01.21.gif NAM.refl_10cm.d01.24.gif NAM.refl_10cm.d01.27.gif NAM.refl_10cm.d01.30.gif NAM.refl_10cm.d01.33.gif NAM.refl_10cm.d01.36.gif NAM.refl_10cm.d01.39.gif NAM.refl_10cm.d01.42.gif NAM.refl_10cm.d01.45.gif NAM.refl_10cm.d01.48.gif NAM.refl_10cm.d01.51.gif NAM.refl_10cm.d01.54.gif NAM.refl_10cm.d01.57.gif NAM.refl_10cm.d01.60.gif NAM.refl_10cm.d01.63.gif NAM.refl_10cm.d01.66.gif NAM.refl_10cm.d01.69.gif NAM.refl_10cm.d01.72.gif front_page_loop.gif");
